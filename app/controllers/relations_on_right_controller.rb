...
# Manage requirements relations list with fixed right side.
class RelationsOnRightController < ApplicationController
  before_action :find_project
  before_action :find_requirement

  crud do
    actions %i[create new]
    attributes %w[kind left_id]
    class_name "Goals::RequirementRelation"
    item_name :relation
    index_path { project_requirement_path(project_id: @project.id, id: @requirement.id) }
    new_path do
      new_project_requirement_right_path(project_id: @project.id, requirement_id: @requirement.id)
    end
  ...
  def delete
    @requirement.relations_on_right.where(id: params[:ids]).delete_all
  ...
  def find_object
    self.object = @requirement.relations_on_right.find(params[:id])
  end
  ...
  def init_object
    self.object = @requirement.relations_on_right.new
  end
  ...
  def requirement_kinds_for_relation_kinds
    AppConfig.requirement_relations.to_h do |key, options|
      requirement_kinds =
        options.requirement_kinds.filter_map { |item| item.left if item.right == @requirement.kind }
